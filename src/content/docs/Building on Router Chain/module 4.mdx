...
After you've had the Project Structure Initialized with you, it is time to Compile & Build the Contracts. Run the following Command in yout Directory to Build the Smart Contracts - 

```bash
cargo build --target wasm32-unknown-unknown --release
```

After Building the Smart Contracts, you need to Deploy them and in CosmWasm that is being done using the `.wasm` file you get in the `artifacts` folder.

## Possible Errors you Might Get

<Aside type="danger">
While Building your Contracts it might give you an error related to - 
